    ...
    internal class EnglishDictionaryProvider : IDictionaryProvider
    {
        ...
        public async Task<List<DictionaryEntry>> LookupAsync(string word, CancellationToken token)
        {
            try
            {
                ...
                var configuredTimeout = TimeSpan.FromSeconds(Math.Max(1, ConfigurationManager.Configuration.HttpTimeoutSeconds));
                primaryTokenSource.CancelAfter(configuredTimeout < MaximumPrimaryTimeout ? configuredTimeout : MaximumPrimaryTimeout);

                return await LookupConfiguredApiAsync(word, primaryTokenSource.Token);
            }
            catch (OperationCanceledException) when (!token.IsCancellationRequested)
            {
                Debug.WriteLine($"[Definition Plugin] Primary English dictionary timed out for '{word}'. Trying Datamuse.");
            }
            catch (HttpRequestException ex)
            {
                Debug.WriteLine($"[Definition Plugin] Primary English dictionary failed for '{word}': {ex.Message}. Trying Datamuse.");
            }
            catch (JsonException ex)
            {
                Debug.WriteLine($"[Definition Plugin] Primary English dictionary returned invalid JSON for '{word}': {ex.Message}. Trying Datamuse.");
            }
        ...
        {
            var endpoint = _apiEndpoint ?? ConfigurationManager.Configuration.ApiEndpoint;
            var requestUrl = $"{endpoint.TrimEnd('/')}/{Uri.EscapeDataString(word)}";
            ...
            {
                return json.RootElement.Deserialize<List<DictionaryEntry>>(options) ?? new List<DictionaryEntry>();
            }

            if (json.RootElement.ValueKind == JsonValueKind.Object)
            {
                var apiResponse = json.RootElement.Deserialize<FreeDictionaryApiResponse>(options);
                return ConvertResponse(apiResponse);
            }

            throw new JsonException("Dictionary service returned an unsupported response shape.");
        }

        private async Task<List<DictionaryEntry>> LookupDatamuseAsync(string word, CancellationToken token)
        {
            var requestUrl = $"{_fallbackApiEndpoint}{Uri.EscapeDataString(word)}&md=d&max=1";
            using var response = await _httpClient.GetAsync(requestUrl, token);
            ...
            await using var jsonStream = await response.Content.ReadAsStreamAsync(token);
            var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
            var words = await JsonSerializer.DeserializeAsync<List<DatamuseDefinitionResponse>>(jsonStream, options, token);
            var match = words?.FirstOrDefault(result => string.Equals(result.Word, word, StringComparison.OrdinalIgnoreCase));
        ...
        private static List<DictionaryEntry> ConvertResponse(FreeDictionaryApiResponse response)
        {
            ...
            var meanings = response.Entries
                ...
                .Select(entry => new Meaning
                {
                    ...
                    Definitions = FlattenSenses(entry.Senses)
